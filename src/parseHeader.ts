 ...
 */
export default function parseHeader(line: string): string {
  ...
  return `<h${level} id="${slug}" style="position: relative; display: flex; align-items: center;">
    <a href="#${slug}" class="header-anchor" style="text-decoration: none; color: inherit; display: flex; align-items: center;" onmouseover="this.querySelector('svg').style.opacity='1'" onmouseout="this.querySelector('svg').style.opacity='0.6'">
      ${linkIcon}
    </a>
    <span>${content}</span>
  </h${level}>`;
